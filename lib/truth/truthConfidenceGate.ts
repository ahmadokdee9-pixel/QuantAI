...
function hasEvidenceReasoningGraphEvidence(sources: TruthEvidenceSources): boolean {
  return sources.hasEvidenceReasoningGraph === true && Boolean(sources.canonicalSkuId);
}
...
function hasDecisionEngineEvidence(sources: TruthEvidenceSources): boolean {
  return sources.hasDecisionEngine === true && Boolean(sources.canonicalSkuId);
}
...
export function computeTruthConfidence(
  ...
  if (
    hasEvidenceReasoningGraphEvidence(sources) &&
    sources.evidenceStrength < WEAK_EVIDENCE_STRENGTH_THRESHOLD
  ) {
    gatesApplied.push("weak_evidence_strength");
  }
  if (
    hasEvidenceReasoningGraphEvidence(sources) &&
    sources.evidenceCompleteness < WEAK_EVIDENCE_COMPLETENESS_THRESHOLD
  ) {
    gatesApplied.push("incomplete_evidence");
  }
  if (
    hasEvidenceReasoningGraphEvidence(sources) &&
  ...
  if (hasDecisionEngineEvidence(sources) && sources.decisionScore < WEAK_DECISION_SCORE_THRESHOLD) {
    gatesApplied.push("weak_decision_score");
  }
  if (hasDecisionEngineEvidence(sources) && sources.decisionConfidence < WEAK_DECISION_CONFIDENCE_THRESHOLD) {
    gatesApplied.push("weak_decision_confidence");
  }
  if (hasDecisionEngineEvidence(sources) && sources.decisionRiskCount >= ELEVATED_DECISION_RISK_THRESHOLD) {
    gatesApplied.push("elevated_decision_risks");
  }
  if (hasDecisionEngineEvidence(sources) && isAvoidDecisionState(sources.decisionState)) {
    gatesApplied.push("avoid_decision_state");
  }
...
export function applyTruthConfidenceGate(args: {
  ...
  if (
    ...
    gates.push("downgrade_weak_evidence_strength");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
  ...
  if (
    ...
    gates.push("downgrade_incomplete_evidence");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
  ...
  if (
    ...
    gates.push("downgrade_conflicting_evidence");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
  ...
  if (enteredAsBuyTier && hasDecisionEngineEvidence(sources) && sources.decisionScore < WEAK_DECISION_SCORE_THRESHOLD) {
    gates.push("downgrade_weak_decision_score");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
  ...
  if (
    enteredAsBuyTier &&
    hasDecisionEngineEvidence(sources) &&
    sources.decisionConfidence < WEAK_DECISION_CONFIDENCE_THRESHOLD
  ) {
    gates.push("downgrade_weak_decision_confidence");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
    confidence = Math.min(confidence, 31);
  }

  if (
    enteredAsBuyTier &&
    hasDecisionEngineEvidence(sources) &&
    sources.decisionRiskCount >= ELEVATED_DECISION_RISK_THRESHOLD
  ) {
    gates.push("downgrade_elevated_decision_risks");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
    confidence = Math.min(confidence, 30);
  }

  if (enteredAsBuyTier && hasDecisionEngineEvidence(sources) && isAvoidDecisionState(sources.decisionState)) {
    gates.push("downgrade_avoid_decision_state");
    tier = downgradeTier(tier, "COMPARE");
    verdict = "COMPARE";
  ...
  return {
    ...
    productIntelligence: {
      ...
      alignmentFlags: [
        ...
        "phase1l_evidence_reasoning_graph",
        ...
        "phase1n_decision_intelligence",
        `phase1n_decision_state_${(truthBundle.sources.decisionState ?? "unknown").toLowerCase()}`,
        `phase1n_decision_score_${truthBundle.sources.decisionScore}`,
        `phase1n_decision_confidence_${truthBundle.sources.decisionConfidence}`,
        "phase2b_intent_retrieval",
        `phase2b_retrieval_score_${truthBundle.sources.retrievalIntentScore}`,
        `phase2b_retrieval_reasons_${truthBundle.sources.retrievalReasons.length}`,
        "phase2c_product_matching",
        `phase2c_overall_match_${truthBundle.sources.overallMatchScore}`,
        `phase2c_brand_match_${truthBundle.sources.brandMatchScore}`,
        "phase2d_product_reasoning",
        `phase2d_recommendation_strength_${(truthBundle.sources.recommendationStrength ?? "unknown").toLowerCase()}`,
        `phase2d_reasoning_confidence_${truthBundle.sources.productReasoningConfidence}`,
        `phase2d_explainability_${truthBundle.sources.explainabilityScore}`,
        `phase2d_evidence_chain_${truthBundle.sources.reasoningEvidenceChain.length}`,
        "phase2e_recommendation_intelligence",
        `phase2e_recommendation_tier_${(truthBundle.sources.recommendationTier ?? "not_recommended").toLowerCase()}`,
        `phase2e_recommendation_score_${truthBundle.sources.recommendationScore}`,
        `phase2e_recommendation_confidence_${truthBundle.sources.recommendationConfidenceScore}`,
        `phase2e_should_recommend_${truthBundle.sources.shouldRecommend}`,
        `phase2e_evidence_chain_${truthBundle.sources.recommendationEvidenceChain.length}`,
        "phase2f_explainable_ai",
        `phase2f_explainability_confidence_${truthBundle.sources.explainabilityConfidence}`,
        `phase2f_final_verdict_${(truthBundle.sources.explainableFinalVerdict ?? "unknown").toLowerCase().replace(/\s+/g, "_").slice(0, 48)}`,
        `phase2f_strengths_${truthBundle.sources.explainableStrengthCount}`,
        `phase2f_weaknesses_${truthBundle.sources.explainableWeaknessCount}`,
        `phase2f_evidence_chain_${truthBundle.sources.explainableEvidenceChain.length}`,
        "phase2g_conversational_intent",
        `phase2g_conversational_confidence_${truthBundle.sources.conversationalConfidence}`,
        `phase2g_expertise_${(truthBundle.sources.conversationalExpertiseLevel ?? "unknown").toLowerCase()}`,
        `phase2g_budget_sensitivity_${(truthBundle.sources.conversationalBudgetSensitivity ?? "unknown").toLowerCase()}`,
        `phase2g_brand_flexibility_${(truthBundle.sources.conversationalBrandFlexibility ?? "flexible").toLowerCase()}`,
        `phase2g_preference_signals_${truthBundle.sources.preferenceSignalCount}`,
        `phase2g_evidence_chain_${truthBundle.sources.conversationalEvidenceChain.length}`,
        "phase2h_taste_preference",
        `phase2h_aesthetic_${(truthBundle.sources.aestheticProfile ?? "balanced").toLowerCase()}`,
        `phase2h_style_${(truthBundle.sources.styleProfile ?? "general").toLowerCase()}`,
        `phase2h_premium_affinity_${truthBundle.sources.premiumAffinity}`,
        `phase2h_value_affinity_${truthBundle.sources.valueAffinity}`,
        `phase2h_taste_confidence_${truthBundle.sources.tasteConfidence}`,
        `phase2h_taste_signals_${truthBundle.sources.tasteSignalCount}`,
        `phase2h_evidence_chain_${truthBundle.sources.tasteEvidenceChain.length}`,
        "phase2i_user_decision_intelligence",
        `phase2i_decision_strategy_${(truthBundle.sources.userDecisionStrategy ?? "bestvalue").toLowerCase()}`,
        `phase2i_decision_confidence_${truthBundle.sources.userDecisionConfidence}`,
        `phase2i_best_value_${truthBundle.sources.bestValueStrategyScore}`,
        `phase2i_best_quality_${truthBundle.sources.bestQualityStrategyScore}`,
        `phase2i_budget_choice_${truthBundle.sources.budgetChoiceStrategyScore}`,
        `phase2i_decision_signals_${truthBundle.sources.userDecisionSignalCount}`,
        `phase2i_evidence_chain_${truthBundle.sources.userDecisionEvidenceChain.length}`,
        "phase2j_purchase_motivation",
        `phase2j_motivation_${(truthBundle.sources.purchaseMotivation ?? "productivity").toLowerCase()}`,
        `phase2j_motivation_confidence_${truthBundle.sources.purchaseMotivationConfidence}`,
        `phase2j_gaming_${truthBundle.sources.gamingMotivationScore}`,
        `phase2j_gifting_${truthBundle.sources.giftingMotivationScore}`,
        `phase2j_luxury_${truthBundle.sources.luxuryMotivationScore}`,
        `phase2j_innovation_${truthBundle.sources.innovationMotivationScore}`,
        `phase2j_motivation_signals_${truthBundle.sources.purchaseMotivationSignalCount}`,
        `phase2j_evidence_chain_${truthBundle.sources.purchaseMotivationEvidenceChain.length}`,
        "phase2k_purchase_constraints",
        `phase2k_constraint_${(truthBundle.sources.primaryConstraint ?? "budget").toLowerCase()}`,
        `phase2k_constraint_confidence_${truthBundle.sources.constraintConfidence}`,
        `phase2k_budget_${truthBundle.sources.budgetConstraintScore}`,
        `phase2k_gaming_${truthBundle.sources.gamingConstraintScore}`,
        `phase2k_brand_${truthBundle.sources.brandConstraintScore}`,
        `phase2k_hard_requirements_${truthBundle.sources.hardRequirementCount}`,
        `phase2k_constraint_signals_${truthBundle.sources.constraintSignalCount}`,
        `phase2k_evidence_chain_${truthBundle.sources.purchaseConstraintsEvidenceChain.length}`,
        "phase1d5_truth_gate",
