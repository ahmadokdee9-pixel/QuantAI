/**
 * Phase 15 — Lifecycle-aware buying strategy.
 */
...
export function buildLifecycleStrategy(args: {
  evolution?: CommerceEvolutionResult | null;
  commerceIdentity?: AutonomousCommerceIdentityResult | null;
}): { phase: string; strategy01: number } {
  const phase =
    args.commerceIdentity?.lifecycleTransition.toPhase ?? args.evolution?.lifecycle.phase ?? "discovery";
  const strategy01 = round4(
    (args.evolution?.lifecycle.lifecycleMaturity01 ?? 0.2) * 0.6 +
      (args.commerceIdentity?.maturity.maturity01 ?? 0.2) * 0.4
  );
  return { phase, strategy01 };
}
