...
export async function refreshLiveMarketProducts(
  ...
  const attemptedQueries = buildExternalExpansionQueries(query, candidates).slice(0, 1);
  ...
  const result = await withTimeout(fetchShoppingProducts(attemptedQueries[0]!), timeoutMs, fallback);
  if (!result.ok) {
    return { products: [], attemptedQueries, timedOut: result.status === 504, source: "serpapi" };
  }
  return {
