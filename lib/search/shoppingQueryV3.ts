...
const FAMILY_SUFFIX: { test: RegExp; add: string }[] = [
  ...
  { test: /\b(perfume|fragrance|parfum|cologne|eau\s+de|libre|edp|ysl|yves\s+saint\s+laurent)\b/i, add: "perfume fragrance eau de parfum" },
  { test: /\bair\s*fryer\b/i, add: "kitchen small appliance" },
  { test: /\b(desk\s+setup|minimalist\s+desk)\b/i, add: "office desk monitor keyboard chair" },
  { test: /\b(like\s+.{2,30}\s+but\s+cheaper|vomero|pegasus)\b/i, add: "running shoes sneakers" },
